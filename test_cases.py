...
def moe_init(es_size):
    # Create a comm prependicular to the pipeline group as gate group
    world_size = dist.get_world_size()
    rank = dist.get_rank()
    for i in range(0, es_size):
        ranks = range(i, world_size, es_size)
        group = torch.distributed.new_group(ranks)
        if rank in ranks:
            ep_group = group
    for i in range(0, world_size, es_size):
        ranks = range(i, i + es_size)
        group = torch.distributed.new_group(ranks)
        if rank in ranks:
            es_group = group
...
class Attention(nn.Module):
    def __init__(self, M,heads,mp_size,mp_group) -> None:
        super().__init__()

        self.layer_norm=torch.nn.LayerNorm(M,eps=1e-5,elementwise_affine=True)
        self.hidden_size=M
        qkv_size_per_partition = (M// mp_size) * 3
        out_size_per_partition = M // mp_size
        
       
        self.attn_qkvw = nn.Parameter(0.001*torch.randn(qkv_size_per_partition,M).cuda(),
                                    requires_grad=True)
    
        self.attn_qkvb = nn.Parameter(torch.zeros(qkv_size_per_partition).cuda(),
                                    requires_grad=True)
        
        self.attn_ow = nn.Parameter(0.001*torch.randn(M,
                                                out_size_per_partition ).cuda(),
                                    requires_grad=True)

        self.attn_ob = nn.Parameter(torch.zeros(M).cuda(),
                                    requires_grad=True)
        
        self.num_attention_heads_per_partition = heads // mp_size
        self.hidden_size_per_partition = M // mp_size
        self.hidden_size_per_attention_head = M // heads

        self.mp_group = mp_group
        self.norm_factor=math.sqrt(M// heads)
...
class GshardGate(nn.Module):
    def __init__(self, k,capacity_factor,M) -> None:
        super().__init__()
        self.top_k = k
        self.capacity_factor = capacity_factor
        self.wg = torch.nn.Linear(M, dist.get_world_size(), bias=False)
...
class Expert(nn.Module):
    def __init__(self, M,H,ffn_type=0) -> None:
        super().__init__()
        self.num_local_expert = 1
        hidden_hidden_size = H
        self.hidden_size = M
        ...
        self.activation_fn = nn.GELU() if ffn_type==0 else nn.SiLU()
...
class MoeTransformer(nn.Module):
    ...
    def __init__(self, M,H,k,capacity_factor,heads,d1,d2,orderT,ffn_type=0,mp_group=None,orders_unit=None):
        super().__init__()
        self.attention=Attention(M,heads,dist.get_world_size(mp_group),mp_group)
        self.mp_group=mp_group
        self.gate = GshardGate(k,capacity_factor,M)
        self.experts = Expert(M,H,ffn_type)
        self.hidden_size = M
        self.layer_norm=torch.nn.LayerNorm(M,eps=1e-5,elementwise_affine=True)
    ...
    def forward(self,inp):
        ...
        if self.d1>1 or self.orderT==1 or (self.d1==1 and self.d2==1):
            return self.forward_with_order(inp)
        ...
        dispatched_input = a2a_ffn_overlap_forward(dispatched_input, expert_fn=expert_fn, a2a_ffn_overlap_degree=self.d2, use_2dh=False, group=_groups["ep_group"])
...
def mp_init(mp_size):
    ...
    num_tp_group=dist.get_world_size() // mp_size
...
def rerange_orders(orders):
    ...
    return out_order
def out_log(B,L,M,H,k,capacity_factor,heads,mp_size,d1,d2,orderT,outtimes,forwardtimes,str_time_sets):
    if dist.get_rank() == 0:
        global output_file
        # print(outtimes)
...
if __name__ == "__main__":
    ...
    def test(B,L,M,H,heads,mp_size,k,capacity_factor,d1,d2,orderT):
        ####read orders
        str_time_sets=""
        order_path="orders64.txt"
        if mp_size==1:
            order_path="orders64_mp1.txt"
        ...
        for line in lines:
            ...
            if ocounts%2==0:
                ...
                if (L==seq)and  m1==M and h1==H and heads1==heads and mp1==mp_size:
                    if d1==d1_set[dcounts] and d2==d2_set[dcounts]:
        ...
        mp_group=mp_init(mp_size)
        x = torch.randn([B, L, M], dtype=torch.float32, device="cuda",requires_grad=True)
        
        model =MoeTransformer(M,H,k,capacity_factor,heads,d1,d2,orderT,ffn_type=0,mp_group=mp_group,orders_unit=OrdersFunc(s,s1,d1,d2,mp_group,B,events_list)).cuda()
        times = []
        ...
        schedule=torch.profiler.schedule(
            ...
            warmup=10,
            active=10),
        on_trace_ready=decorate_trace_handler( dist.get_rank())
        ) 
        ...
        if args.measure_time:
            ...
            xsplit1=x[:xshape[0]//d1]
            xsplit2=x[:xshape[0]//d2]
            ...
            out_log(B,L,M,H,k,f,heads,mp_size,d1,d2,orderT,0,0,str_time_sets)
            return 
        # ####get default order
        ...
        if mp_size==1 :
            if d1==d2:
                tem_orders=[0,1,2,3]*d1+[4,5]*d2+[6]*d2+[7]*d2
            elif d1>d2:
                tms=d1//d2
                tem_orders=([0,1]*tms+[2,3])*d2+[4,5]*d2+[6]*d2+[7]*d2
            else:
                tms=d2//d1
                tem_orders=([0,1]+[2,3]*tms)*d1+[4,5]*d2+[6]*d2+[7]*d2
                            ...
                            for d1 in [8,4,2,1]:
                                for d2 in [8,4,2,1]:
                                    for orderT in [0,1]:
                                        iter_count += 1 
                                        if iter_count >= 2050:
                                            test(B,L,M,H,heads,mp_size,k,capacity_factor,d1,d2,orderT)
                                            # torch.distributed.barrier()
                                            # time.sleep(5)
                                            # torch.cuda.synchronize()
