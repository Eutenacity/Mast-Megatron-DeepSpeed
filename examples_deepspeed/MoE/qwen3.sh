...
HIDDEN_SIZE=2048
NUM_ATTN_HEADS=32
GLOBAL_BATCH_SIZE=$(expr $NNODES \* 4)
# LR=2.0e-4
# MIN_LR=2.0e-5
...
# TRAIN_ITERS=100
TRAIN_ITERS=$(( ${TRAIN_TOKENS} * 3 / ${GPUS_PER_NODE} / ${SEQ_LEN} ))
...
PP_SIZE=1
NUM_GPUS=$(expr $NNODES \* $GPUS_PER_NODE)
###############################################################################
### MoE configs
...
PYTHON_PAHT=$2
CODE_PAHT=$3
run_cmd="$PYTHON_PAHT -m torch.distributed.launch --nproc_per_node=${GPUS_PER_NODE}   --nnodes=${NNODES} --node_rank=${NODE_RANK} --master_addr=${MASTER_ADDR} --master_port=${MASTER_PORT} $CODE_PAHT/pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options}"
# echo ${run_cmd}
eval ${run_cmd}
set +x
