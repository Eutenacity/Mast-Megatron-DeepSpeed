    ...
    def backward(ctx, grad_output):
        ...
        if len(grad_output.shape) == 3:
            grad_output = grad_output.view(grad_output.shape[0] * grad_output.shape[1],
                                        grad_output.shape[2])
            total_input = total_input.contiguous().view(total_input.shape[0] * total_input.shape[1],
                        total_input.shape[2])
        else:
...
class ColumnParallelLinear(torch.nn.Module):
    ...
    def forward(self,
                input_: torch.Tensor,
                weight: Optional[torch.Tensor] = None,all_reduce=True):
...
class RowParallelLinear(torch.nn.Module):
    ...
    def forward(self, input_,all_reduce=True):
        ...
        elif self.is_expert_without_slicing or all_reduce==False: # non-expert only tensor-parallelism
